...
const pages = fs.readdirSync(".").filter(f => f.endsWith(".html")).map(file => {
...
const newsPages = pages.filter(p => /^(index|intelligence|disha|article-12|digital-constitutional-personhood|media|books|about|intelligence-)/.test(p.file.replace(/\.html$/, ""))).slice(0, 1000);
...
const feedItems = pages.slice(0, 40).map(p => `<item><title>${escapeXml(p.title)}</title><link>${escapeXml(p.url)}</link><guid>${escapeXml(p.url)}</guid><pubDate>${new Date(p.modified).toUTCString()}</pubDate><description>${escapeXml("Public-interest record from thenitishkr.in")}</description></item>`).join("\n");
const feed = `<?xml version="1.0" encoding="UTF-8"?>\n<rss version="2.0"><channel><title>thenitishkr.in public-interest archive</title><link>${SITE}/</link><description>Research records, case files, DISHA, Article 12, and digital governance archive.</description><lastBuildDate>${new Date().toUTCString()}</lastBuildDate>${feedItems}</channel></rss>\n`;
